...
const FullMaintenanceSuiteIndex = ({ auth, maintenanceTasksz, Vehicles }) => {
    ...
    const handleTaskAction = (taskId, action) => {
        post(
            route("markAsComplete", {
                id: taskId,
                VID: selectedVH ? selectedVH.id : 11,
            })
        );
    };
    useEffect(() => {
    ...
    function smoothScroll(frame) {
        ...
        if (window.scrollY === 0) cancelAnimationFrame(fps);
    }
    ...
    useEffect(() => {
        setData({
            rep: { title: "Full Maintenance Suite", dataMain: DataMain },
            user: auth.user,
        });
                                                ...
                                                (vehicle, index) => (
                                                    <li key={index}>
                                                        <div className="flex p-4 justify-center items-center">
                                                            ...
                                                            <label
                                                                htmlFor={`VID${index}`}
                                                                className="ml-2 select-none cursor-pointer hover:underline decoration-sky-500"
                                                            >
                                                                {vehicle.make}
                                                            </label>
                                                        </div>
                                                        ...
                                                        {(auth.user.role ===
                                                            ...
                                                            <div className="flex space-x-2">
                                                                <button
                                                                    ...
                                                                    className={`text-white px-3 py-1 rounded bg-${
                                                                        task.status ===
                                                                        1
                                                                            ? "gray"
                                                                            : "green"
